...
function Scheduler(options) {
  ...
  // URL prefix
  this.prefix = options.prefix || '/unison';
...
Scheduler.prototype.init = function init() {
  ...
  this.router.get(this.prefix + '/jobs', this.getJobs.bind(this));
  this.router.get(this.prefix + '/map/:id', this.getMap.bind(this));
  this.router.put(this.prefix + '/solution/:id', this.handleSubmit.bind(this));

  // Emit broadcasted events
  var self = this,
      subscriber = this.db.getSubscriber();

  subscriber.on('pmessage', function(pattern, channel, message) {
    if (pattern === self.db.prefix + 'j:new/*') {
      self.emit('job:new', message);
      return;
    }
  });
  subscriber.psubscribe(this.db.prefix + 'j:new/*');

  this.on('job:saved', function(job) {
    self.db.publish(self.db.prefix + 'j:new/' + job.id, job.id);
  });
};
...
// #### @req {Request}
// #### @res {Response}
// PUT /unison/solution/:id handler
//
