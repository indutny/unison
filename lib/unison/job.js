...
Job.prototype.save = function save(callback) {
  ...
  transaction.exec(function(err, results) {
    ...
    }, db.multi()).exec(function(err) {
      scheduler.emit('job:saved', self);
      callback(err);
    });
  });
};
...
Job.prototype.remove = function remove(dequeueOnly, callback) {
  ...
  transaction.exec(function(err, results) {
    ...
    if (!members) return callback(err);

    var success = results.every(function(result) {
      return result != 0;
    });
    if (!success) {
      // Delete all related stuff anyway, but report error
      err = new Error('Job was already removed');
    }
    ...
    members.forEach(function(rid) {
      ...
      transaction.zrem(prefix.set, rid + '/' + self.qid());
    });
    transaction.del(prefix.replica + self.qid());
    transaction.exec(function(err2) {
      callback(err2 || err);
    });
  });
};
...
Job.prototype.addSolution = function addSolution(rid, solution, callback) {
  ...
  async.waterfall([function(callback) {
    ...
    async.waterfall([function(callback) {
      ...
      // Remove job
      self.remove(true, function(err) {
        if (err) return callback(err);

        // Notify listeners about result
        db.publish(prefix.complete + self.qid(), out);
        callback(null, true);

        // And restart job if it's continuous
        if (!self.continuous) return;
        try {
          self.input = JSON.parse(out);
        } catch (e) {
        }
        self.iteration++;
        scheduler.add(self);
...
Job.loadRandom  = function loadRandom(N, callback) {
  ...
  async.whilst(function() {
    ...
    db.srandmember(prefix.queue, function(err, item) {
      ...
      // If no itme can be found - wait for it to appear
      if (!item) {
        return scheduler.once('job:new', callback);
      }
...
subscriber.on('pmessage', function(pattern, channel, message) {
  var match = channel.match(channels.complete);
  if (match !== null) {
    var id = match[1],
